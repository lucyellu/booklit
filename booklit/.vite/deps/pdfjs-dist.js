...
function wrapReason(ex) {
	...
	switch (ex.name) {
		...
		case "MissingPDFException": return new MissingPDFException(ex.message);
		case "PasswordException": return new PasswordException(ex.message, ex.code);
		case "UnexpectedResponseException": return new UnexpectedResponseException(ex.message, ex.status);
...
function getDocument(src = {}) {
	...
	worker.promise.then(function() {
		...
		if (rangeTransport) networkStream = new PDFDataTransportStream(rangeTransport, {
			...
			let NetworkStream;
			if (isNodeJS) if (isValidFetchUrl(url)) {
