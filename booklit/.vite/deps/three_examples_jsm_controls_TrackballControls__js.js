import { $r as MathUtils, Bs as Vector3, Yr as MOUSE, _a as Quaternion, vt as Controls, zs as Vector2 } from "./three.module-Dhi4sXJn.js";
//#region node_modules/three/examples/jsm/controls/TrackballControls.js
/**
	...
	*/
	...
	_panCamera() {
		...
		if (_mouseChange.lengthSq()) {
			...
			if (this.staticMoving) this._panStart.copy(this._panEnd);
			else this._panStart.add(_mouseChange.subVectors(this._panEnd, this._panStart).multiplyScalar(this.dynamicDampingFactor));
		}
	}
	_rotateCamera() {
	...
	_zoomCamera() {
		...
		if (this.state === _STATE.TOUCH_ZOOM_PAN) {
			...
			if (this.object.isPerspectiveCamera) this._eye.multiplyScalar(factor);
			else if (this.object.isOrthographicCamera) {
			...
			if (factor !== 1 && factor > 0) if (this.object.isPerspectiveCamera) this._eye.multiplyScalar(factor);
			else if (this.object.isOrthographicCamera) {
				this.object.zoom = MathUtils.clamp(this.object.zoom / factor, this.minZoom, this.maxZoom);
				if (this._lastZoom !== this.object.zoom) this.object.updateProjectionMatrix();
			} else console.warn("THREE.TrackballControls: Unsupported camera type");
			if (this.staticMoving) this._zoomStart.copy(this._zoomEnd);
			else this._zoomStart.y += (this._zoomEnd.y - this._zoomStart.y) * this.dynamicDampingFactor;
		}
	}
	...
	_getMouseOnCircle(pageX, pageY) {
		_v2.set((pageX - this.screen.width * .5 - this.screen.left) / (this.screen.width * .5), (this.screen.height + 2 * (this.screen.top - pageY)) / this.screen.width);
		return _v2;
	}
	_addPointer(event) {
	...
	_removePointer(event) {
		...
		for (let i = 0; i < this._pointers.length; i++) if (this._pointers[i].pointerId == event.pointerId) {
...
function onKeyDown(event) {
	...
	else if (event.code === this.keys[_STATE.ROTATE] && !this.noRotate) this.keyState = _STATE.ROTATE;
	else if (event.code === this.keys[_STATE.ZOOM] && !this.noZoom) this.keyState = _STATE.ZOOM;
...
function onMouseMove(event) {
	...
	if (state === _STATE.ROTATE && !this.noRotate) {
		this._movePrev.copy(this._moveCurr);
		this._moveCurr.copy(this._getMouseOnCircle(event.pageX, event.pageY));
	} else if (state === _STATE.ZOOM && !this.noZoom) this._zoomEnd.copy(this._getMouseOnScreen(event.pageX, event.pageY));
	else if (state === _STATE.PAN && !this.noPan) this._panEnd.copy(this._getMouseOnScreen(event.pageX, event.pageY));
}
function onMouseUp() {
	this.state = _STATE.NONE;
...
function onMouseWheel(event) {
	...
	switch (event.deltaMode) {
		case 2:
			this._zoomStart.y -= event.deltaY * .025;
			break;
		case 1:
			this._zoomStart.y -= event.deltaY * .01;
			break;
		default:
...
//# sourceMappingURL=three_examples_jsm_controls_TrackballControls__js.js.map